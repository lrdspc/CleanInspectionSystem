  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        // Informações do cliente e da inspeção
        ...
        createFormattedParagraph(`FAR/Protocolo: ${inspection.protocolNumber}`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Assunto: AT - BRA - PERMEABILIDADE - Telhado com vazamento Geral`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Elaborado por: ${inspection.technicianName}`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Departamento: ${inspection.department}`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Unidade: ${inspection.unit}`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Coordenador Responsável: ${inspection.coordinator}`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Gerente Responsável: ${inspection.manager}`, { spacing: { before: 120, after: 120 } }),
        createFormattedParagraph(`Regional: ${inspection.region}`, { spacing: { before: 120, after: 120 } }),
        ...
        ...(inspection.issues || []).map((issue, index) => {
          ...
          const issueImage = inspection.issueImages?.find(img => img.issueType === issue);
          if (issueImage?.imageUrl) {
            try {
              // Converter base64 para buffer
              const base64Data = issueImage.imageUrl.split(',')[1];
              ...
              sections.push(
                ...
                // Legenda da imagem
                createFormattedParagraph(issueImage.caption || `Foto do problema: ${issue}`, {
                  spacing: { before: 120, after: 240 },
                  alignment: AlignmentType.CENTER,
                })
              );
            } catch (error) {
              console.error(`Erro ao processar imagem para ${issue}:`, error);
            }
          }
